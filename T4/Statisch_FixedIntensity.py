...
sI_2 = np.array([0.0393, 0.0496, 0.0694, 0.089])   

Delta_theta_2 = np.array([1, 2, 5, 7]) * (2/360) * np.pi    # 
sDelta_theta_2 = np.array(4*[np.sqrt(2*(1**2))])*(2/360)*np.pi  # afleesfout is 2 graden, delta = begin - eind dus fout delta = sqrt(2^2+2^2) graden
...
sv_2 = v_2 * np.sqrt( (sDelta_theta_2/Delta_theta_2)**2 + (sB_2/B_2)**2 + (sl_2/l_2)**2 )
